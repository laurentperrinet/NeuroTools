...
class AnalogSignal(unittest.TestCase):
    ...
    def testMaskEvents(self):
        sig = analogs.AnalogSignal(numpy.ones(10000), 0.1)
        spikes = [0.,50.,100.,995.]
        res1 = sig.mask_events(spikes,t_min=5, t_max=10)
        # check mask transitions, near boarders as well
        assert numpy.alltrue(res1.signal.mask[1099:1101] ==[True,False])
        assert numpy.alltrue(res1.signal.mask[0:101] == [True]*100+[False])
        assert numpy.alltrue(res1.signal.mask[9899:] == [False]+[True]*100)
